...
const AddMoneyWallet = () => {
  ...
  const { cartData, dehliveryAddress } = useSelector(
  ...
  const showPaymentFailureAlert = () => {
    Alert.alert(
      ...
      [
        {
          text: "OK",
          onPress: () => console.log("Payment Failure OK Pressed"),
        },
      ],
      { cancelable: false }
    );
  ...
  const getSession = (orderId, token) => {
    ...
    return new CFSession(token, orderId, CFEnvironment.PRODUCTION);
  };
  ...
  const _startCheckout = async (orderId, token) => {
    try {
      ...
      console.log(
        "recharge_wallet Session:",
        session,
        "recharge_wallet Payment Modes:",
        paymentModes
      );
      ...
      <View style={styles.AddMoneyWalletwrapper}>
        <View style={styles.Head}>
          ...
          <Text style={styles.Points}>
            PTs {walletData?.total_balance.toFixed(1)}
          </Text>
        </View>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TransactionCard
                ...
                imageSource={AppImages.Reward_Bonus_Badge}
