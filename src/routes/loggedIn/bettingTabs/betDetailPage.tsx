...
export function BetDetail({
  navigation,
  route,
}: NativeStackScreenProps<LoggedInStackParamsList, "betDetail">) {
  const { deviceAddress, auth, token, user, deviceActivity, isSkipped } =
  ...
  return (
    <Div bg={baseColors.white} h="100%">
      ...
      <ScrollDiv pt={20} mb={24} px={20} h={hp(100)}>
        ...
        >
          <BetDetailLabel
            title={`PTs ${betDetails?.put_points ?? ""}`}
            subTitle="You Give"
          >
            <FontAwesome5
          ...
          <BetDetailLabel
            title={`PTs ${betDetails?.get_points ?? ""}`}
            subTitle="You Get"
          >
            <FontAwesome5
              name="hand-holding-usd"
