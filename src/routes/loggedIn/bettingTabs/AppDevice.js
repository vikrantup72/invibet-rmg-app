        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Block
              item={item}
              token={token}
              onDisconnectPress={(id) => onDisconnectBtn(id)}
...
const Block = ({
  ...
  return (
    <Pressable
      style={[
        style.listinner,
        { paddingVertical: hp(1), opacity: isConnected ? 1 : 0.0 },
      ]}
      onPress={() => onDisconnectPress(item?.device_id)}
    >
      ...
      <Text
        style={[
          style.connected,
          { color: isConnected ? baseColors.theme : "red" },
        ]}
      >
...
export function AppDevice({ navigation }) {
  ...
  const data = [{ title: "Connect with Application", route: "" }];
  ...
  const disconnect = async (deviceId) => {
    ...
    Alert.alert(
      "Disconnect Device",
      "Would you like to disconnect the device?",
      [
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const response = await fetch(
                "https://invibet-backend-280569053519.asia-south1.run.app/profile/disconnect/device",
                {
                  method: "PUT",
                  headers: {
                    Authorization: ` ${token}`,
