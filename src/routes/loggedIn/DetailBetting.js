    ...
    <View style={styles.ActivityProgresscontainer}>
      ...
      <View style={styles.textContainer}>
        ...
        <Text style={styles.value}>
          {statusMessage == "Starts in" || statusMessage == ""
            ? "-"
            : statusMessage == "Ends in"
            ? statusMessageBooster == "Ends in" && countdownBooster > 0
              ? `${step}/10000`
              : step
            : leaderboardSteps}
...
const BetDetail = ({ targetSteps = 10000, calories = 250 }) => {
  ...
  useEffect(() => {
    if (betDetails) {
      const { totalPrice, deduction, winningAmount } = CalculateWinningPrice(
        parseInt(betDetails?.put_points),
        parseInt(betDetails?.participating)
      );
      setPriceDetail({
  ...
  useEffect(() => {
    if (leaderboard?.length > 0) {
      let UserLeaderboardData = leaderboard?.filter(
    ...
    about: [
      {
        ...
        description:
          "Points are based on your steps and activity during the week. Use boosters strategically to climb the leaderboard.",
        icon: "",
      },
      {
        title: "💰 Entry & Cashback Usage:",
        description:
          "Use your cashback to enter the Mega Pool. Turn your steps and efforts into exciting rewards.",
        icon: "",
      },
      {
        title: "🚀 Boost Your Points:",
        description:
          "Get a 2x Points Booster to maximize your chances. Buy 1 booster per week.",
        icon: "",
      },
      {
        title: "📝 Key Notes:",
        description:
          "Weekly reset, make every step count! Cashback entry is non-refundable.",
        icon: "",
      },
    ],
    joinPoints: 49,
  ...
  useEffect(() => {
    if (betDetails?.booster_start && betDetails?.booster_expiry) {
      setTimeout(() => {
        getCountdownTimeBooster(
          betDetails.booster_start,
          betDetails.booster_expiry
        );
      }, 2000);
    }
          ...
          >
            ...
            {betDetails?.is_joined && (
              <View
                style={{
                  alignSelf: "flex-end",
                  backgroundColor: "background: rgba(43, 186, 79, 0.8)",
                  borderRadius: 30,
        ...
        <ScrollView scrollEnabled={false} style={{ paddingHorizontal: wp(5) }}>
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Image
                ...
                style={{ height: wp(3), width: wp(3), marginRight: 3 }}
              />
              <Text style={styles.titleCountdown}>
                {statusMessage ?? ""} :{" "}
              </Text>
            </View>
          ...
          <View style={styles.titleSection}>
            ...
            <Text style={styles.members}>
              {betDetails?.participating} members
            </Text>
          </View>
          <View
            style={{
          ...
          <View style={styles.statsSection}>
            ...
            <View style={styles.statItem}>
              ...
              {betDetails?.bet_type == "mega_pool" ||
              betDetails?.bet_type == "private_pool" ? (
                <Text style={styles.statValue}>{`₹ ${
                  parseInt(betDetails?.pool_prize) ?? ""
                }`}</Text>
              ) : (
                <Text style={styles.statValue}>{`₹ ${
                  priceDetail?.winningAmount ?? ""
                }`}</Text>
              )}
            </View>
            <Separator />
            ...
            <View style={styles.statItem}>
              ...
              >
                {betDetails?.bet_type == "mega_pool" ||
                betDetails?.bet_type == "private_pool"
                  ? parseInt(betDetails?.pool_prize) / 2 ?? 0
                  : parseInt(priceDetail?.winningAmount) ?? 0}
              </Text>
            </View>
          </View>
              ...
              <View style={styles.bottomActivity}>
                <View style={{ width: "70%" }}>
                  <View
                    style={{
                      marginTop:
                        `${targetSteps}` - `${betDetails?.steps}` != 0
                          ? hp(1)
                          : 0,
                    }}
                  >
                    {statusMessage != "" && (
                      <ActivityProgress
                        step={setps_cureent_day ?? ""}
                        countdown={countdown}
                        statusMessage={statusMessage}
                        leaderboardSteps={userLeaderboardSteps}
                        statusMessageBooster={statusMessageBooster}
                        countdownBooster={countdownBooster}
                      />
                    )}
                  </View>
                </View>
          ...
          {betDetails?.is_joined && (
            <>
              {betDetails?.booster_id != null &&
                !isBoosterExpired(betDetails?.booster_expiry) &&
                timeLeftToEndbooster > 0 && (
                  <View style={styles.activeBoosterMain}>
                    <FastImage
                      source={AppImages.checkGreen}
                      resizeMode="contain"
                      style={styles.checkGreen}
                    />
                    <Text style={styles.boosterActivated}>
                      Booster Activated
                    </Text>
                    <View style={{ marginTop: 1, flexDirection: "row" }}>
                      <Text
                        style={[
                          styles.boosterActivated,
                          { color: "red", fontFamily: AppFonts.semibold },
                        ]}
                      >
                        {" "}
                        {statusMessageBooster} :{" "}
              ...
              {betDetails?.booster_id != null &&
                ...
                timeLeftToEndbooster <= 0 && (
                  <View style={styles.activeBoosterMain}>
                    <Text style={styles.boosterActivated}>
                      ⏳ Booster Used! New One Available Next Week
                    </Text>
                  </View>
                )}
            </>
          ...
          {betDetails?.is_joined && (
            <>
              {betDetails?.booster_id == null && all_boosters?.length > 0 && (
                    ...
                    renderItem={({ item, index }) => {
                      return (
                        <View style={{ marginTop: index == 0 ? 0 : hp(2.5) }}>
                          ...
                          >
                            Note :{" "}
                            <Text style={{ fontFamily: AppFonts.medium }}>
                              2x steps will be counted if you complete 10k steps
                              in 24hr.
                            </Text>
                          </Text>
                  ...
                  {leaderboard?.map((item, index) => {
                    ...
                    let titleMegaPool =
                      item?.rank == 1
                        ...
                        ? `🥉${item.rank}`
                        : `    ${item.rank}`;
                    let titleFloatingPool =
                      item?.rank == 1 ? `🥇${item.rank}` : `    ${item.rank}`;
                    return (
                      <SingleRankBlock
                        item={item}
                        key={item.user_id}
